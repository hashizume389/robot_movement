...
class RotateInPlaceNode(Node):
    """
    SCOUT MINIがその場で5回転（時計回り）して停止するノード。
    その間のオドメトリデータをCSVに記録する。
    """
    def __init__(self):
        super().__init__('rotate_in_place_node')
        
        # ▼▼▼ 変更点: パラメータと状態の定義 ▼▼▼
        # パラメータ（定数）
        # 1回転 = 2πラジアン。 5回転 = 5 * 2 * π
        self.target_angle_ = -5.0 * 2.0 * math.pi # 目標回転角度 (rad) (-1800度)
        self.rotation_tolerance_ = 0.05 # 回転停止の許容誤差 (rad, 約3度)
        ...
        self.initial_position_ = None # オドメトリ受信確認用
        ...
        self.get_logger().info(
            f'ノード起動: その場で5回転 (目標: {self.target_angle_:.2f}rad) します...'
        )
    ...
    def odom_callback(self, msg: Odometry):
        ...
        # ▼▼▼ 変更点: 累積回転角を計算 ▼▼▼
        # このロジックは前回のノードから変更ありません。
        # 複数回転を正しくトラッキングするために重要です。
    ...
    def timer_callback(self):
        ...
        twist_msg = Twist() # linear.x は 0.0 (その場回転)
