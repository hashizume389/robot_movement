...
class MoveSquareNode(Node):
    """
    SCOUT MINIが「10m前進 -> 90度時計回り」を4回繰り返し、
    正方形の軌道を描くノード。
    その間のオドメトリデータをCSVに記録する。
    """
    def __init__(self):
        super().__init__('move_square_node')
        
        # パラメータ（定数）
        self.target_distance_ = 2.0  # 1辺の目標距離 (m)
        self.target_angle_ = -math.pi / 2.0 # 目標回転角度 (rad) (-90度)
        self.rotation_tolerance_ = 0 # 回転停止の許容誤差 
        self.max_loops_ = 4           # ループ回数
        
        self.linear_velocity_ = 0.3    # 前進速度 (m/s) (元の0.3から戻しました)
        ...
        self.get_logger().info(
            f'ノード起動: [10m前進 -> 90度回転] x 4回 を実行します...'
        )
    ...
    def timer_callback(self):
        ...
        if self.current_state_ == self.STATE_MOVING_FORWARD:
            ...
            if self.distance_traveled_ < self.target_distance_:
                ...
                self.get_logger().info(
                    f'前進中 ({self.loop_count_ + 1}/{self.max_loops_}辺目): {self.distance_traveled_:.2f}m / {self.target_distance_}m',
                    throttle_duration_sec=1
                )
            else:
                # 目標達成: 停止し、カウンターを増やす
                self.loop_count_ += 1
                self.get_logger().info(f'目標距離 {self.target_distance_}m に到達。({self.loop_count_}/{self.max_loops_})')
        ...
        # --- 状態 2: 90度時計回り ---
            ...
            # (目標角度 - 許容誤差) よりもまだ大きいか？ (例: -1.50 > (-1.57 + 0.03))
            if self.angle_traveled_ > (self.target_angle_ + self.rotation_tolerance_):
                # まだ回り足りない: 時計回りに回転
                twist_msg.angular.z = -self.angular_velocity_ # 負のZが時計回り
                self.get_logger().info(
                    f'回転中 ({self.loop_count_}/{self.max_loops_}回目): {self.angle_traveled_:.2f}rad / {self.target_angle_:.2f}rad',
                    throttle_duration_sec=1
                )
            else:
                # 目標達成: 停止し、次の状態へ
                ...
                self.current_state_ = self.STATE_MOVING_FORWARD
                self.get_logger().info('状態遷移: 前進 (MOVING_FORWARD)')
                # 次の前進のため、初期位置をリセット
                self.initial_position_ = self.current_position_
                self.distance_traveled_ = 0.0
